...
import { StyleSheet, Text, TouchableNativeFeedback, ScrollView, View } from 'react-native';
    ...
    render() {
        ...
        return ( <ScrollView>
            ...
            <View style={styles.contenedor}>
                ...
                <View style={styles.columna}>
                    {filtradosChapa.filter( vehiculo => vehiculo.tarifa === 'Estadia' ).map( vehiculo => ( <Tarjeta24
                            key={vehiculo.horaIngreso}
                            {...vehiculo}/> ) )
                    }
                </View>
                <View style={styles.columna}>
                    {filtradosChapa.filter( vehiculo => vehiculo.tarifa === 'Doctor' ).map( vehiculo => ( <TarjetaDoc key={vehiculo.horaIngreso} {...vehiculo}/> ) )}
                </View>
